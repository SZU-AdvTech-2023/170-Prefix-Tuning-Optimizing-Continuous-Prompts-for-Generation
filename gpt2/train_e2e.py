...
if __name__ == '__main__':
    ...
    # Added by MX
    parser.add_argument('--cache_dir', type=str, default='../models', help='cache dir')
    parser.add_argument('--use_custom_teacher_dropout', type=str, default='no', help='')
    
    parser.add_argument('--train_and_eval', type=bool, default=False)
    ...
    if args.mode == 'data2text':
        ...
        if args.notes is not None and 'lowdata' in args.notes:
            _, temp_size = args.notes.split('_')
            TRAIN_FILE = "../data/e2e_data/lowdata_{}.txt".format(temp_size)
            print(TRAIN_FILE)
            # TEST_FILE = "/juice/u/xlisali/e2e_lowdata/lowdata_{}_{}_valid.txt".format(temp_seed, temp_size)
            folder_name = 'e2e_lowdata_models_{}/'.format(args.tuning_mode) # 50, 200
    ...
    if args.notes is not None and 'lowdata' in args.notes:
        ...
        app += '--evaluate_during_training --eval_steps 10000 '
    ...
    COMMANDLINE="python /yuhongf/wangyufeng/PrefixTuning/gpt2/run_language_modeling.py \
        --output_dir={} \
        --model_type=gpt2 \
        --model_name_or_path={} \
        ...
        --train_embs no ".format(Model_FILE, "../models/gpt2-medium-s3",args.bsz, args.bsz, args.epoch, TRAIN_FILE, args.mode, TEST_FILE,
